...
def run_one_task(task: Task) -> bool:
    ...
    try:
        ...
        if globals.load_cache is not None:
            ...
            run_ok = inference.run_one_task(task_output_dir, api_manager, problem_stmt, reproducer_res)
        if run_ok:
...
def generate_test_code(task: Task) -> str:
    """
...
#     Extract and execute the code from the provided text.
#     Determines if the extracted code is Python or Shell script and executes it accordingly.
#     Returns formatted output including the test code and the result.
#     """
...
#         return f"reproduce result: {result.stdout}"
    
#     except Exception as e:
#         # Return the formatted traceback as a string if execution fails
#         return f"reproduce result: {traceback.format_exc()}"
